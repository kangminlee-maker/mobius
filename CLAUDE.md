...
- ★ **날짜 파티션 프루닝 — 매 틱 전수 walk 금지**(2026-07-16 수정): 루트가 `YYYY/MM/DD`라
  전체(49K/19GB)를 `FileManager.enumerator`로 훑으면 `getattrlistbulk`가 매 3초 틱마다 CPU를
  태운다(실측: 유휴 앱 CPU **19~21%**, `sample` 스택에서 getattrlistbulk가 최다). tick의
  로그 스캔은 15초 게이트 밖이라 실제로 매 3초 돈다 — "0.1s라 15초 틱에 허용"이라던 기존
  실측은 디렉토리가 커지며 낡았다(0.25s+, cold면 더). → `SessionLogWatcher.recentDirs` 주입:
  **최근 N일(Codex=7일) 날짜 폴더만 열거**(신규 파일 발견) + **추적 중인 파일은 폴더 나이와
  무관하게 직접 확인**(옛 폴더 resume의 append 유지). 실측 효과: 열거 49,296→7,494개(6.6배↓),
  최근 수정분 누락 0, 유휴 CPU 19%→**0.1%**, getattrlistbulk 3702→108 샘플. 잔여(문서화된
  트레이드오프): N일보다 오래 유휴였던 미추적 파일이 resume되면 발견 못 함 — 계정 한도는
  계정 전역이라 활성/최근 세션 이벤트로 교차 반영된다. Claude 워처는 프로젝트별 구조라
  프루닝 미적용(전체 열거 유지, 부하 작음). 상세: `SessionLogWatcher.codexLookbackDays`.
