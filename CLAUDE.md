...
Sources/MobiusCore/       앱·CLI 공유 코어 (전부 의존성 주입 → 테스트 가능)
  ...
  ResetProber.swift        리셋 프로브 판단부 — 소진→리셋 지난 계정 선별, 창당 1회/백오프
  ResetProbe.swift         프로브 실행기 — Claude(최소 API 호출+usage 폴링) / Codex(exec --json
                           → thread_id로 rollout 파일 직접 파싱, 활성 계정 전용)
...
- ★ **`codex resume`는 며칠 지난 원본 rollout 파일에 이어 쓴다**(실측: 7/8 파일이 7/12 갱신).
  ...
  의존). 삭제된 파일만 추적 해제.
- ★ **날짜 파티션 프루닝 — 매 틱 전수 walk 금지**(2026-07-16 수정, 07-17 프라이밍 시딩 보강):
  루트가 `YYYY/MM/DD`라 전체(49K/19GB)를 `FileManager.enumerator`로 훑으면 `getattrlistbulk`가
  매 3초 틱마다 CPU를 태운다(실측: 유휴 앱 CPU **19~21%**, `sample` 스택 최다 getattrlistbulk).
  tick의 로그 스캔은 15초 게이트 밖이라 실제로 매 3초 돈다 — "0.1s라 15초 틱에 허용"이라던 기존
  실측은 디렉토리가 커지며 낡았다(0.25s+, cold면 더). → `SessionLogWatcher.recentDirs` 주입:
  **최근 N일(Codex=7일, +내일까지) 날짜 폴더만 열거**(신규 발견) + **추적 중인 파일은 폴더
  나이와 무관하게 직접 확인**(옛 폴더 resume의 append 유지). **첫 스캔(프라이밍)은 프루닝 없이
  전수 열거**해 창 밖 옛 폴더의 최근 파일까지 시딩한다 — offsets가 메모리 전용이라 재시작/오프라인
  후 옛 세션 resume이 끊기지 않게(리뷰 반영, 1회 비용, 파싱 없이 오프셋만). 실측: 열거
  49,296→7,494개(6.6배↓), 최근 수정분 누락 0, 유휴 CPU 19%→**0.1%**, getattrlistbulk 3702→108
  샘플. 잔여(축소된 트레이드오프): **프라이밍 이후 실행 중 처음 resume되는** N일+ 옛 폴더 세션만
  못 봄(재시작 시 재시딩) — 계정 한도는 계정 전역이라 활성/최근 세션 이벤트로 교차 반영된다.
  Claude 워처는 프로젝트별 구조라 프루닝 미적용(전체 열거 유지, 부하 작음). 상세:
  `SessionLogWatcher.codexLookbackDays`.
...
- **설정 UI 재구성 + 자동 전환 풀별 분리(2026-07-12,
  ...
  테스트 103개 green.
- **리셋 프로브 구현됨(2026-07-13, 설계·게이트 실측은 2026-07-12
  ...
  (워처 tailOnly가 일회성 턴을 놓치므로). 유닛 11개 추가(총 114 green).
  미검증 잔여: 실제 리셋 이벤트로 tick 전체 경로 라이브 관측(기능 켠 뒤 첫 소진→리셋 때).
