...
- **[정정 2026-07-14] P3는 "프리미엄 모델(Fable) 지출 한도"다 — 위 2026-07-13 "세션 계속
  동작"의 실체는 저가 모델만 동작** — 실측(사용자): P3에 걸린 계정은 **프리미엄 모델(Fable)이
  막히고**, 5h/주간 창에 여유가 있으면 **저가 모델(Sonnet/Haiku)만 계속 동작**한다. 메인테이너
  해석(월 지출 = 프리미엄 한도)이 맞다. → **동작 방향 재설정**: P3 시 **비핀 계정은 폴백으로
  옮겨** 프리미엄을 다른 계정에서 유지하는 게 낫고, **핀 계정은 사용자의 명시적 선택이므로
  머문다**. (구현은 monthly-spend PR — 창 소진과 겹칠 때 `exhaustionHit`가 modelScoped를
  버리지 않도록 핀 상호작용을 함께 손봐야 함. 현재는 "여유면 무시"라 비핀 프리미엄 유지가 안 됨.)
